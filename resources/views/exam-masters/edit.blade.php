...
.info-note { background:#fff7ed; border:1px solid #fed7aa; color:#9a3412; padding:10px 12px; border-radius:6px; font-size:12px !important; line-height:1.5; }
...
<div style="background:#fff;border-radius:12px;padding:20px;border:1px solid #d1d5db;box-shadow:0 4px 10px rgba(0,0,0,.15);">
    ...
    <h2 class="text-center text-blue-600 mb-4">Edit Exam</h2>
    ...
    @php
        $examTypesList   = ['UNIT TEST 1','UNIT TEST 2','UNIT TEST 3','UNIT TEST 4','TERM 1','TERM 2','ANNUAL'];
        $currentExamType = $examTypeValue ?? '';
    @endphp

    <form method="POST" action="{{ route('exam-masters.update', $examMaster->id) }}" id="examEditForm">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="academic_year_id" id="academic_year_id" class="w-full border rounded p-2" required>
                    ...
                    @foreach($academicYears as $y)
                        <option value="{{ $y->id }}" {{ (string) old('academic_year_id', $examMaster->academic_year_id) === (string) $y->id ? 'selected' : '' }}>{{ $y->year_name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="standard_id" id="standard_id" class="w-full border rounded p-2" required>
                    ...
                    @foreach($standards as $s)
                        <option value="{{ $s->id }}" {{ (int) old('standard_id', $examMaster->standard_id) === (int) $s->id ? 'selected' : '' }}>{{ $s->standard_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-5">
            <div>
                ...
                <select id="exam_type" name="exam_type" class="w-full border rounded p-2" required>
                    ...
                    @foreach($examTypesList as $t)
                        <option value="{{ $t }}" {{ $currentExamType === $t ? 'selected' : '' }}>{{ ucwords(strtolower($t)) }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block font-semibold mb-2">Exam Name</label>
                <input type="hidden" name="exam_name" id="exam_name" value="{{ old('exam_name', $examMaster->exam_name) }}">
                <input type="text" id="exam_name_preview" value="{{ old('exam_name', $examMaster->exam_name) }}" readonly class="w-full border rounded p-2 bg-gray-100">
            </div>
        </div>
        ...
        <div class="border rounded section-box mb-6 bg-gray-50" style="padding:12px;">
            <div class="flex justify-between items-center mb-3">
                ...
                <span id="subjectLoading" class="text-blue-600" style="display:none;">Loading...</span>
            </div>
            <div id="passingPercentageNote" class="passing-percentage-note"></div>
            <div class="overflow-x-auto">
                <table class="subj-tbl">
                    ...
                    <tbody id="subjectTableBody">
                        <tr><td colspan="10" class="border p-3 text-center text-gray-500">Loading...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="mb-5">
            ...
            <input type="number" name="display_order" value="{{ old('display_order', $examMaster->display_order) }}" readonly class="w-full border rounded p-2 bg-gray-100">
        </div>

        <div class="mb-5">
            <label class="flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $examMaster->is_active) ? 'checked' : '' }}>
                <span>Active</span>
            </label>
        </div>

        <div class="info-note mb-5">
            <strong>Important:</strong> Subject marks reload from the Academic Exam Structure whenever Standard or Exam Type changes. Save to apply.
        </div>

        <div class="flex justify-between items-center mt-6">
            <div></div>
            <div class="flex gap-2">
                <button type="submit" class="erp-btn erp-btn-save" id="updateExamButton">Update</button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const updBtn = document.getElementById('updateExamButton');
    ...
    function buildExamName() {
        ...
        if (!type || !stdT) return;
        eName.value = type + ' - ' + stdT;
        ePrev.value = eName.value;
    }

    async function loadSubjects() {
        const stdId = std.value, examT = et.value;
        if (!stdId || !examT) return;
    ...
    form.addEventListener('submit', function (e) {
        ...
        buildExamName();
        updBtn.disabled = true; updBtn.innerText = 'Updating...';
    });

    if (std.value && et.value) loadSubjects();
});
</script>
